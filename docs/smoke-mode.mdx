...
teams end up hand-rolling a bespoke MCP test client to check their own
...
Every check prints a line, passes included. A tool that answered gets a
`returned` line saying what came back: content block count and types, whether
`structuredContent` was present, `isError`, and the first 80 characters of the
first text block, whichever check made the call. `Tools called: 3 of 4` counts
tools the server answered at least once (a timeout or a dropped connection is
not an answer); the fourth was never invoked because it lacks `readOnlyHint: true`.
...
- **Not available on [mcpscore.dev](https://mcpscore.dev)**, because
  invoking tools on third-party URLs from our infrastructure is what the
  security posture forbids. Smoke mode is CLI-only. In the
  [GitHub Action](/github-action), `--smoke` in `args` fails the job on a
  smoke failure but drops the report and comment, since the action treats
  any non-zero exit as a failed audit. A native input is on the way.
...
<CardGroup cols={2}>
  <Card title="GitHub Action" icon="github" href="/github-action">
    Gate PRs on the score; native smoke support is on the way
  </Card>
