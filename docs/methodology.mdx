...
3. **Probe** — a set of sessionless requests observing
   ...
   **mcpscore never invokes `tools/call` against your server**, so an audit can never
   ...
   version, a bogus session ID, a request for a nonexistent resource, and one
   truncated JSON body. These can look like probing in a WAF or access log; they
   read state and nothing more. Each one exists to check a documented MUST — for
   example, the spec requires HTTP 403 for an invalid `Origin`, HTTP 404 with
   JSON-RPC `-32601` for an unknown method, and JSON-RPC requires malformed JSON
   to produce `-32700` with a null ID.
