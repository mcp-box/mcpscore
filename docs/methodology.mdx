...
3. **Probe** — a set of sessionless requests observing
   ...
   **mcpscore never invokes `tools/call` against your server**, so an audit can never
   ...
   through the SDK are probed over stdio too. The suite launches two short-lived
   sibling processes through the MCP SDK's stdio transport and sends no
   `initialize` handshake: one runs the probe suite, while the other provides an
   independent connection for catalog-stability comparisons. The truncated-JSON
