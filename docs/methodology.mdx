...
3. **Probe** — a set of sessionless requests observing
   ...
   **mcpscore never invokes `tools/call` against your server**, so an audit can never
   ...
   version, a bogus session ID, and a request for a nonexistent resource. These
   can look like probing in a WAF or access log; they read state and nothing more.
   Each one exists to check a documented MUST — for example, the spec requires
   HTTP 403 for an invalid `Origin` and HTTP 404 with JSON-RPC `-32601` for an
   unknown method.

   The modern lifecycle is stateless, so its JSON-RPC behaviors are probed over
   stdio too. The suite launches one short-lived sibling process through the MCP
   SDK's stdio transport and sends no `initialize` handshake. Questions that only
   exist in HTTP—such as `Origin`, headers, status codes, sessions, and auth
   metadata—are recorded as not applicable for stdio rather than failed.
...
| `readiness_2026_jsonrpc_batch_rejected` | MEDIUM | Each POST body MUST contain a single JSON-RPC message, not a batch ([Streamable HTTP](https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http#sending-messages)) |
| `readiness_2026_malformed_json_rejected` | MEDIUM | Malformed JSON must be rejected as invalid client input rather than accepted or surfaced as a server failure ([Streamable HTTP](https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http#sending-messages)) |
