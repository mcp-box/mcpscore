...
| `insufficient-data` | The observations the rule needs are unavailable (e.g. a probe errored, or the probe's subject doesn't exist on this transport) — the rule can neither pass nor fail |
...
3. **Probe** (HTTP/HTTPS servers only) — a set of sessionless requests observing
   ...
   **stdio servers are probed too.** The 2026-07-28 lifecycle is stateless — a
   request carries its own `_meta` envelope and needs no handshake — so "does
   this server speak the new revision?" is a JSON-RPC question, answerable over
   stdio. Each stdio probe launches its own short-lived server process, separate
   from the audit's session. The probes whose *subject* is an HTTP construct
   (`Origin`, `Mcp-Session-Id`, header/body agreement, the well-known auth
   documents) are recorded as not applicable rather than run.
