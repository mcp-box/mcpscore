...
without userinfo, query, or fragment, and rule `details`, which can hold the
audited URL verbatim, stay in the JSON report. Keep credentials in `--token`
or `--header` rather than the URL, and the alert identity stays stable across
token rotations.
