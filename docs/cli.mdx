...
code carry it. The file carries no credentials: a URL target is written
without userinfo, query, or fragment, every URL quoted in a rule message is
written the same way, a `--stdio` command shows only its program and script,
and rule `details`, which can hold the audited URL verbatim, stay in the JSON
report. Nothing hidden enters a hash either: the alert identity blanks the
values of credential-looking query parameters and drops command-line option
values. Keep credentials in `--token` or `--header` rather than the URL all
the same, and the alert identity stays stable across token rotations.
