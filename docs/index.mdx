...
- **A severity-weighted score** across the same categories the report and
  [mcpscore.dev](https://mcpscore.dev) show — Protocol, Primitives (tools,
  prompts, resources, resource templates, pagination), and Security & Auth —
  deterministic, no API keys, CI-ready.
- **A readiness score for the latest spec revision** (2026-07-28, stateless
  lifecycle) — reported on its own axis. For legacy servers it informs without
  punishing; servers already on the modern lifecycle have these points counted
  in the main score (see [promotion](/methodology#the-readiness-score-separate-axis)).
...
```bash
# Local servers (stdio, including modern-only stateless servers)
...
# Gate any CI on a threshold: exit code 3 when the score percentage is below it
