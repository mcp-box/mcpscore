...
target names its own analysis category (`mcpscore/<target>/`), so a re-run
updates alerts in place and two servers audited in one workflow stay two
sets. The upload step's `category` input has no effect here: the file already
