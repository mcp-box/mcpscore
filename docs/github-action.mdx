...
an alert in the repository's Security tab, alongside the findings of your
other scanners. The upload needs `security-events: write`, and in a private
repository `contents: read` as well; the audit itself needs nothing more
than before.
...
carries its findings. Alerts are keyed on the rule and the target, and each
target names its own analysis category (`mcpscore/<target>/<digest>/`, with
`partial` or `incomplete` added when the audit was one), so a re-run updates
alerts in place, two servers audited in one workflow stay two sets, and a
partial audit never closes a full audit's alerts. The upload step's
`category` input has no effect here: the file already
carries its category, and `upload-sarif` sets one only when it is missing. The
findings are about the running server, not a line of source, so they appear in
the Security tab rather than as annotations in the pull request diff; the
`min-score` gate is what fails the pull request.
