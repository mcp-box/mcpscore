...
from mcpscore.probes import ProbeOutcome, ProbeResult, has_modern_support
...
def _probes_observed(probes: dict[str, ProbeResult] | None) -> bool:
    """Whether any probe actually reached the server.
    ...
    asked. Neither is evidence that a server lacks modern support.
    """
    return any(
        result.outcome not in (ProbeOutcome.NOT_APPLICABLE, ProbeOutcome.ERROR) for result in (probes or {}).values()
    )
