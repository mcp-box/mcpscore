...
- **`--sarif FILE` writes the failed rules as SARIF 2.1.0 for GitHub code
  ...
  opening a new one. The file carries no credentials: a URL target is shown
  without userinfo, query, or fragment, and rule `details` stay in the JSON
  report. Security & Auth rules carry
...
- **MCP Python SDK bumped to `mcp==2.2.0`** (from 2.1.1). The bump itself
  ...
  - **Smoke mode's structured-content check** now also reports a tool whose
    ...
    instead of trying to fetch the reference, which the check surfaces as
    `declared outputSchema is not honored: Invalid schema for tool …`.
  - **`--oauth` is stricter on the legacy discovery path** (servers without
    protected-resource metadata): authorization-server metadata whose
    `issuer` is not the server's own origin is rejected, a 403 that is not
    an `insufficient_scope` challenge is returned instead of retried, and a
